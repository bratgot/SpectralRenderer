...
EDITS_KERNEL = [
    ("GPU kernel: log first rays' NdotV + doubleSided decision",
     KERNEL_OLD, KERNEL_NEW,
     "MPDiagKernel: px=%d py=%d NdotV"),
]
...
def main():
    ...
    print("grab stderr. Look for 'MPDiag:' / 'MPDiagKernel:' lines:")
    print("")
    print("  Flow check:")
    ...
    print("       sees the flag. If matCheck returns same value both toggles,")
    print("       the flag never changed on SpectralMaterial.")
    print("    4. 'MPDiagKernel: px=X py=Y NdotV=Z matId=W doubleSided=V'")
    print("       shows what the kernel actually sees per-ray.")
