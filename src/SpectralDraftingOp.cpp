// ---------------------------------------------------------------------------
// SpectralDraftingOp.cpp -- artistic wireframe material
// ---------------------------------------------------------------------------
...
const char* const SpectralDraftingOp::CLASS = "SpectralDrafting";

static Op* build(Node* node) { return new SpectralDraftingOp(node); }
const Op::Description SpectralDraftingOp::description(CLASS, build);

SpectralDraftingOp::SpectralDraftingOp(Node* node)
    : ShaderOp(node)
{
}
...
{
    return
        "SpectralDrafting -- artistic line-art / wireframe overlay material.\n\n"
...
void SpectralDraftingOp::knobs(Knob_Callback f)
{
    ShaderOp::knobs(f);

    Text_knob(f,
        "<b><font size='+1'>SpectralDrafting</font></b><br>"
...
bool SpectralDraftingOp::test_input(int idx, Op* op) const
{
    // Accept only ShaderOp-derived ops at input 0.
    if (idx != 0 || !op) return false;
    return dynamic_cast<ShaderOp*>(op) != nullptr;
}

const char* SpectralDraftingOp::input_label(int n, char* /*buffer*/) const
{
    return (n == 0) ? "shader" : nullptr;
}

int SpectralDraftingOp::knob_changed(Knob* k)
{
    RegisterParams();
    return ShaderOp::knob_changed(k);
}

std::unordered_map<std::string, SpectralDraftingOp::DraftingParams>&
SpectralDraftingOp::GetRegistry()
{
    static std::unordered_map<std::string, DraftingParams> s_registry;
    return s_registry;
}
...
void SpectralDraftingOp::RegisterParams()
{
    ...
    // Pass-through: capture upstream ShaderOp's node_name so the consumer
    // (SpectralRenderIop) can match the bound material by the upstream's
    // path substring instead of our own.
    if (inputs() > 0 && input(0)) {
        if (ShaderOp* up = dynamic_cast<ShaderOp*>(input(0)))
            p.upstreamName = up->node_name();
    }

    GetRegistry()[node_name()] = p;
}

void SpectralDraftingOp::_SetShaderProperties(usg::ShaderDesc& desc,
...
usg::ShaderDesc* SpectralDraftingOp::createShaderGraph(
    int32_t                outputType,
    const MaterialContext& rtx,
    usg::ShaderDescGroup&  shaderGroup)
{
    // Pass-through mode: if an upstream ShaderOp is connected, delegate
    // material generation to it so the mesh binds to the base shader's
    // material. We still register our own wireframe params; the overlay
    // pass matches the upstream shader's name at render time.
    if (inputs() > 0 && input(0)) {
        if (ShaderOp* up = dynamic_cast<ShaderOp*>(input(0))) {
            usg::ShaderDesc* delegated =
                up->createShaderGraph(outputType, rtx, shaderGroup);
            RegisterParams();
            return delegated;
        }
    }

    // Standalone mode (original behaviour): produce the transparent
    // stub material so the wireframe overlay has a surface to sit on.
    ...
    if (!desc) {
        fprintf(stderr, "SpectralDrafting: failed to create UsdPreviewSurface schema\n");
        return nullptr;
    }
...
void SpectralDraftingOp::updateShaderGraphOverrides(
    int32_t                outputType,
    const MaterialContext& rtx,
    usg::ShaderDescGroup&  shaderGroup)
{
    // Delegate to upstream when chained -- don't clobber the base shader's
    // overrides with our stub values.
    if (inputs() > 0 && input(0)) {
        if (ShaderOp* up = dynamic_cast<ShaderOp*>(input(0))) {
            up->updateShaderGraphOverrides(outputType, rtx, shaderGroup);
            RegisterParams();
            return;
        }
    }
