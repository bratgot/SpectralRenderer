...
void SpectralRenderIop::knobs(Knob_Callback f)
{
    ...
    {
        Bool_knob(f, &_vpEnableShadows, "vp_enable_shadows", "shadows");
        ClearFlags(f, Knob::STARTLINE);
        Tooltip(f, "Enable the viewport shadow-map pass.\n\n"
                   "OFF by default because the shadow pass iterates all\n"
                   "triangles through OpenGL immediate mode every frame,\n"
                   "causing tumble stutter on dense geometry (e.g. subdivided\n"
                   "meshes, character rigs). Turn ON for still-frame lighting\n"
                   "review, OFF for interactive viewport navigation.");
    }
    {
...
{
    fprintf(stderr, "TumbleDiag: _validate enter, forReal=%d\n", forReal ? 1 : 0);
    // --------------------------------------------------------------------
    ...
    for (const UsdPrim& prim : stage->Traverse()) {
        ...
        if (prim.HasAPI<UsdLuxLightAPI>()) {
            ...
            if (light.enableColorTemperature) {
                ...
                // Dome (environment) lights represent a CAPTURED colour --
                // either an HDRI or a tinted ambient. The near-white ->
                // D65 heuristic below is meant for directional sun/sky,
                // where D65 gives more realistic spectral behaviour. For
                // dome it would replace the user's near-white ambient with
                // full CIE daylight, producing a blown-out white sky on
                // CPU spectral renders while GPU shows the intended hue.
                // Always use RGB for dome so CPU matches GPU.
        ...
        if (prim.IsA<UsdGeomPointInstancer>()) {
            ...
            for (size_t inst = 0; inst < positions.size(); ++inst) {
                ...
                // PointInstancer <-> SpectralMeshProperties: the regular
                // mesh path below consults the MeshProperties registry,
                // but this PointInstancer block ran on its own and always
                // defaulted to visible. Inline-query here so that the
                // visible checkbox on a MeshProperties node bound to
                // instanced geometry actually hides it. Same substring
                // + size()==1 matching the main path uses.
                bool instVisible = true;
                {
                    std::string instPath = prim.GetPath().GetString();
                    const auto& instMeshReg = SpectralMeshPropertiesOp::GetRegistry();
                    for (const auto& instEntry : instMeshReg) {
                        if (instPath.find(instEntry.first) != std::string::npos
                            || instMeshReg.size() == 1) {
                            instVisible = instEntry.second.visible;
                            break;
                        }
                    }
                }
                meshData.visible = instVisible;
                ...
                if (surfaceShader) {
                    ...
                    // Check for SpectralDrafting node assigned to this material
                    {
                        ...
                        for (const auto& entry : wireReg) {
                            bool matched = shaderPath.find(entry.first) != std::string::npos;
                            // Pass-through: also match against the upstream
                            // ShaderOp's name, because when Drafting chains
                            // through another shader the bound material path
                            // reflects the upstream node, not Drafting.
                            if (!matched && !entry.second.upstreamName.empty()) {
                                matched = shaderPath.find(entry.second.upstreamName) != std::string::npos;
                            }
                            if (matched) {
        ...
        if (!data.triangles.empty()) {
            ...
            // SpectralMeshProperties flip-normals + orientation: both
            ...
            // Stamp per-mesh doubleSided onto the materials this mesh
...
{
    fprintf(stderr, "TumbleDiag: append enter\n");
    ...
    hash.append(_vpEnableShadows ? 1 : 0);
    ...
    // scrubbed the timeline. Classic CLAUDE.md gotcha #1.
    {
        ...
        for (const auto& kv : wireReg) {
            ...
            hash.append(p.upstreamName.c_str());
        }
    }
...
// ---------------------------------------------------------------------------
//  _EnsureGeoVBO -- build or rebuild the viewport geometry VBO.
//  Called at the top of draw_handle when the scene has changed.
//  Also recomputes cached scene bounds in the same pass so we avoid a
//  second 120k-triangle iteration later in the frame.
// ---------------------------------------------------------------------------
void SpectralRenderIop::_EnsureGeoVBO()
{
    const size_t totalTris = _scene ? _scene->TotalTriangles() : 0;

    // Nothing to upload? Release any existing VBO and bail.
    if (totalTris == 0) {
        if (_glGeoVBO) { glDeleteBuffers(1, &_glGeoVBO); _glGeoVBO = 0; }
        _glGeoVBOTriCount = 0;
        _glGeoMeshRanges.clear();
        _glSceneBoundsValid = false;
        _glGeoVBODirty = false;
        return;
    }

    // Only rebuild if the triangle count changed or something explicitly
    // dirtied us. For now triangle count is the only signal -- if meshes
    // deform but keep the same count we'd miss it. That matches the rest
    // of the viewport path (shadow map also doesn't re-detect deform).
    if (!_glGeoVBODirty && _glGeoVBOTriCount == totalTris && _glGeoVBO != 0) {
        return;
    }

    // Build interleaved buffer: { float3 pos, float3 norm, float2 uv }
    // stride 32 bytes, 3 verts per tri.
    const size_t vertCount  = totalTris * 3;
    const size_t floatsPerV = 8;   // 3 + 3 + 2
    std::vector<float> interleaved;
    interleaved.reserve(vertCount * floatsPerV);
    ...
    for (const auto& kv : _scene->GetMeshes()) {
        const auto& meshData = kv.second;
        if (!meshData.visible || meshData.triangles.empty()) continue;
        int matId = meshData.triangles[0].materialId;
        GeoMeshRange r{ firstTri, (int)meshData.triangles.size(), matId };
        _glGeoMeshRanges.push_back(r);
        firstTri += (int)meshData.triangles.size();

        for (const auto& tri : meshData.triangles) {
            // Flat face normal (matches the non-scanline_compat path)
            pxr::GfVec3f e1 = tri.v1 - tri.v0;
            pxr::GfVec3f e2 = tri.v2 - tri.v0;
            pxr::GfVec3f fn(e1[1]*e2[2]-e1[2]*e2[1],
                            e1[2]*e2[0]-e1[0]*e2[2],
                            e1[0]*e2[1]-e1[1]*e2[0]);
            // Choose vertex vs face normal. We upload vertex normals when
            // the triangles have them (scanline_compat smooth-shaded
            // viewport), else face normal.
            bool hasVN = !_scanlineCompat
                ? false
                : (tri.n0[0]!=0 || tri.n0[1]!=0 || tri.n0[2]!=0);
            const pxr::GfVec3f& n0 = hasVN ? tri.n0 : fn;
            const pxr::GfVec3f& n1 = hasVN ? tri.n1 : fn;
            const pxr::GfVec3f& n2 = hasVN ? tri.n2 : fn;

            // Bounds
            for (int a = 0; a < 3; ++a) {
                if (tri.v0[a] < mn[a]) mn[a] = tri.v0[a];
                if (tri.v1[a] < mn[a]) mn[a] = tri.v1[a];
                if (tri.v2[a] < mn[a]) mn[a] = tri.v2[a];
                if (tri.v0[a] > mx[a]) mx[a] = tri.v0[a];
                if (tri.v1[a] > mx[a]) mx[a] = tri.v1[a];
                if (tri.v2[a] > mx[a]) mx[a] = tri.v2[a];
            }

            // v0
            interleaved.push_back(tri.v0[0]); interleaved.push_back(tri.v0[1]); interleaved.push_back(tri.v0[2]);
            interleaved.push_back(n0[0]);      interleaved.push_back(n0[1]);      interleaved.push_back(n0[2]);
            interleaved.push_back(tri.uv0[0]); interleaved.push_back(1.f - tri.uv0[1]);
            // v1
            interleaved.push_back(tri.v1[0]); interleaved.push_back(tri.v1[1]); interleaved.push_back(tri.v1[2]);
            interleaved.push_back(n1[0]);      interleaved.push_back(n1[1]);      interleaved.push_back(n1[2]);
            interleaved.push_back(tri.uv1[0]); interleaved.push_back(1.f - tri.uv1[1]);
            // v2
            interleaved.push_back(tri.v2[0]); interleaved.push_back(tri.v2[1]); interleaved.push_back(tri.v2[2]);
            interleaved.push_back(n2[0]);      interleaved.push_back(n2[1]);      interleaved.push_back(n2[2]);
            interleaved.push_back(tri.uv2[0]); interleaved.push_back(1.f - tri.uv2[1]);
        }
    }

    // Upload
    if (!_glGeoVBO) glGenBuffers(1, &_glGeoVBO);
    glBindBuffer(GL_ARRAY_BUFFER, _glGeoVBO);
    ...
    _glGeoVBOTriCount = totalTris;
    _glSceneMin[0]=mn[0]; _glSceneMin[1]=mn[1]; _glSceneMin[2]=mn[2];
    _glSceneMax[0]=mx[0]; _glSceneMax[1]=mx[1]; _glSceneMax[2]=mx[2];
    _glSceneBoundsValid = true;
    _glGeoVBODirty = false;
    SLOG("SpectralRender: viewport VBO rebuilt, %zu tris\n", totalTris);
}

// Note: _IsInteractiveDraw (detect tumble for skipping shadow pass) lives
// in Part B of this fix. We need to verify the correct Nuke NDK event
// enum before shipping; guessing risks a worse bug than the one we're
// fixing. For Part A we only add VBO infrastructure.


void SpectralRenderIop::draw_handle(ViewerContext* ctx)
{
    static int s_drawHandleCallCount = 0;
    if ((++s_drawHandleCallCount % 30) == 1) {
        fprintf(stderr, "TumbleDiag: draw_handle call #%d\n", s_drawHandleCallCount);
    }
    ...
    if (_scene && _scene->TotalTriangles() > 0) {
        ...
        if (_glGeoProg) {
            ...
            // Gated on _vpEnableShadows; when off we skip the entire pass
            // for interactive viewport performance. See vp_enable_shadows
            // knob and related notes in SpectralRenderIop.h.
            float lightVP[16] = {};
            bool hasShadowMap = false;
            if (_vpEnableShadows
                && _glShadowFBO && _glShadowDepthProg && _glShadowDepthTex && gHasLight) {
...
void SpectralRenderIop::_BuildLightRig()
{
            ...
            if (hdriIop) {
                try {
                    ...
                    if (W > 0 && H > 0) {
                        ...
                        // Sample 5 pixels from local `tex` BEFORE move into scene.
    ...
    for (SpectralStudioLight* sl : _allStudioLights) {
        ...
        if (ki > 0.001) {
            ...
            // Respect the user's keyColor knob (all 47 presets tune this).
            // Previously this was hardcoded to (ki, ki*0.95, ki*0.9) -- a
            // fixed amber tint -- so preset colours had no visual effect.
            key.color = GfVec3f(sl->keyColor[0] * float(ki),
                                sl->keyColor[1] * float(ki),
                                sl->keyColor[2] * float(ki));
            key.illuminant = SpectralLight::Illuminant::RGB;
            key.intensity = _lightIntensity;
            _scene->AddLight(key);
        }
        ...
        if (fi > 0.001) {
            ...
            // Slight lift on fill elevation: keyEl * 0.5 could dip the
            // fill below horizon for low-key setups. Use 0.6 with a
            // 5-degree floor so fills stay gentle but never underground.
            double fillEl = std::max(5.0, sl->keyElevation * 0.6);
            fill.direction = dirFromElevAzim(fillEl, sl->keyAzimuth + 180);
            // Respect user's fillColor knob.
            fill.color = GfVec3f(sl->fillColor[0] * float(fi),
                                 sl->fillColor[1] * float(fi),
                                 sl->fillColor[2] * float(fi));
            fill.illuminant = SpectralLight::Illuminant::RGB;
            fill.intensity = _lightIntensity;
        ...
        if (ri > 0.001) {
            ...
            // Respect user's rimColor knob.
            rim.color = GfVec3f(sl->rimColor[0] * float(ri),
                                sl->rimColor[1] * float(ri),
                                sl->rimColor[2] * float(ri));
            rim.illuminant = SpectralLight::Illuminant::RGB;
            rim.intensity = _lightIntensity;
            _scene->AddLight(rim);
        }
    }
...
void SpectralRenderIop::_EnsureFrameRendered()
{
    ...
    if (_aovPRef || _wireframeEnable)     _pRefBuffer.assign(size_t(W) * H * 3, 0.f);   else _pRefBuffer.clear();
...
void SpectralRenderIop::_RenderStrip(int stripIdx)
{
    ...
    if (!_frameReady.load()) {
...
#endif
        {
            ...
            aovBufs.pRef     = (_aovPRef || _wireframeEnable) ? _pRefBuffer.data() : nullptr;
            ...
            if (needGeomPass) {
                SpectralIntegrator::ComputeGeometryAOVs(
                    *_scene, _renderCam,
                    ...
                    (_aovPRef || _wireframeEnable) ? _pRefBuffer.data() : nullptr,
        ...
        if (_wireframeEnable && float(_wireOpacity) > 0.001f && !_uvBuffer.empty()) {
            ...
            const bool  hasPRef     = !_pRefBuffer.empty();
                ...
                for (int x = 0; x < W; ++x) {
                    ...
                    // ── UV-grid based styles (0-4) ──
                    {
                        ...
                        // ── Style 1: Guide (thin dashed construction lines) ──
                            ...
                            // Dash ALONG the edge: pick the UV axis the edge
                            // lies on (whichever distN is smaller) and use
                            // the OTHER axis as the parameter that varies
                            // along the line. Divide by pixelUVSize so the
                            // dash knobs stay in pixel-equivalent units.
                            float alongUV = (distU < distV) ? v : u;
                            float dashUnit = alongUV / pixelUVSize;
                            float period = float(_wireDashLength) + float(_wireGapLength);
                            float dashPos = std::fmod(std::abs(dashUnit), period);
                    ...
                    // Dash runs ALONG the edge: whichever UV axis the edge
                    // lies on picks the OTHER as the along-line parameter.
                    // distU/distV from the inner UV-grid block went out of
                    // scope when that block closed, so recompute locally.
                    if (_wireDashed && (style == 0 || style == 3)) {
                        float _gu = u * gridFreq;
                        float _gv = v * gridFreq;
                        float _dU = std::abs(_gu - std::round(_gu)) / gridFreq;
                        float _dV = std::abs(_gv - std::round(_gv)) / gridFreq;
                        float alongUV = (_dU < _dV) ? v : u;
                        float dashUnit = alongUV / pixelUVSize;
                        float period = float(_wireDashLength) + float(_wireGapLength);
                        float dashPos = std::fmod(std::abs(dashUnit), period);
                        if (dashPos > float(_wireDashLength)) continue;
