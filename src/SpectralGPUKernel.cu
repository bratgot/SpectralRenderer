...
// ---------------------------------------------------------------------------
//  Metal Fresnel -- per-wavelength measured n,k
//  Tabulated at 380,430,480,530,580,630,680,730,780 nm. Matches the table
//  in SpectralBSDF.h::_GetMetalIOR so CPU and GPU render metals identically.
//  Sources: Palik's Handbook of Optical Constants.
//  Indexed: 0=none, 1=gold, 2=copper, 3=silver, 4=aluminium, 5=iron, 6=titanium
// ---------------------------------------------------------------------------
__device__ static const float kMetalN[7][9] = {
    {1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f},                                     // 0: none
    {1.66f,1.47f,1.29f,0.64f,0.28f,0.17f,0.16f,0.16f,0.17f},                   // 1: gold
    {1.27f,1.21f,1.14f,1.08f,0.47f,0.23f,0.21f,0.21f,0.24f},                   // 2: copper
    {0.05f,0.04f,0.04f,0.04f,0.05f,0.06f,0.07f,0.10f,0.12f},                   // 3: silver
    {0.51f,0.57f,0.63f,0.72f,0.96f,1.24f,1.55f,1.80f,2.08f},                   // 4: aluminium
    {1.72f,1.86f,2.01f,2.18f,2.39f,2.56f,2.68f,2.78f,2.86f},                   // 5: iron
    {1.60f,1.72f,1.86f,2.00f,2.14f,2.30f,2.48f,2.64f,2.76f},                   // 6: titanium
};
__device__ static const float kMetalK[7][9] = {
    {0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f},                                     // 0
    {1.94f,1.95f,1.85f,2.20f,2.81f,3.24f,3.62f,4.00f,4.38f},                   // 1: gold
    {2.14f,2.37f,2.56f,2.60f,2.65f,2.98f,3.32f,3.67f,4.01f},                   // 2: copper
    {1.95f,2.35f,2.82f,3.29f,3.59f,3.92f,4.25f,4.58f,4.90f},                   // 3: silver
    {4.40f,4.84f,5.30f,5.74f,6.18f,6.62f,7.04f,7.48f,7.90f},                   // 4: aluminium
    {2.88f,2.94f,3.01f,3.08f,3.16f,3.27f,3.39f,3.50f,3.60f},                   // 5: iron
    {2.37f,2.48f,2.58f,2.68f,2.78f,2.90f,3.03f,3.14f,3.24f},                   // 6: titanium
};

// Lookup per-wavelength n,k with linear interpolation across the 9 samples.
static __forceinline__ __device__ void metalNK(int metalType, float lambda,
                                               float& n, float& k)
{
    if (metalType <= 0 || metalType >= 7) { n = 1.f; k = 0.f; return; }
    float t = (lambda - 380.f) / 50.f;
    if (t < 0.f) t = 0.f;
    if (t > 7.99f) t = 7.99f;
    int i = int(t);
    float frac = t - float(i);
    n = kMetalN[metalType][i] * (1.f - frac) + kMetalN[metalType][i+1] * frac;
    k = kMetalK[metalType][i] * (1.f - frac) + kMetalK[metalType][i+1] * frac;
}

// Unpolarised Fresnel reflectance for conductor (n + ik) at angle cosI.
// Returns [0,1] reflectance weighted by the base colour's spectral
// reflectance (kept outside this function so callers can tint).
static __forceinline__ __device__ float fresnelMetal(int metalType,
                                                     float lambda, float cosI)
{
    if (metalType <= 0) return 0.f;
    float n, k;
    metalNK(metalType, lambda, n, k);
    float cosI2 = cosI * cosI;
    float n2k2  = n*n + k*k;
    float twoNc = 2.f * n * cosI;
    float Rs_num = n2k2 - twoNc + cosI2;
    float Rs_den = n2k2 + twoNc + cosI2;
    float Rs = Rs_num / fmaxf(1e-8f, Rs_den);
    float twoNcI = 2.f * n * cosI;
    float n2k2c2 = n2k2 * cosI2;
    float Rp_num = n2k2c2 - twoNcI + 1.f;
    float Rp_den = n2k2c2 + twoNcI + 1.f;
    float Rp = Rp_num / fmaxf(1e-8f, Rp_den);
    float F = 0.5f * (Rs + Rp);
    if (F < 0.f) F = 0.f;
    if (F > 1.f) F = 1.f;
    return F;
...
static __forceinline__ __device__ float3 getHitNormal()
{
    ...
    // normal of the time-interpolated triangle instead. Orient it to face the ray
    // (the visible side) rather than the stale base-pose normal -- past ~90 deg of
    // rotation the base normal points the wrong way and would flip us back to black.
    if (params.geoNormalsForMotion) {
        ...
        if (dot3(gn, rd) > 0.f) gn = neg3(gn);   // face the viewer / incoming ray
        return gn;
    }
    return sn;
}
    ...
    }
    {
        ...
        if (params.scanlineCompat) {
            ...
            for (int s = 0; s < spp; ++s) {
                ...
                makeRay(px+jx, py+jy, W, H, origin, dir, seed + 50u, &rt);

                unsigned int p0=0,p1=0,p2=0,p3=__float_as_uint(1e30f),p4=0,p5=0,p6=0;
                optixTraverse(params.traversable, origin, dir, 1e-4f,1e30f,rt,
