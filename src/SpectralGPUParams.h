...
struct GPUMaterial {
    ...
    // Metal type: 0=none (use Schlick), 1=gold, 2=copper, 3=silver,
    // 4=aluminium, 5=iron, 6=titanium. When metallic > 0.5 the kernel
    // uses a per-wavelength n,k table for proper metal Fresnel -- without
    // this, all metals looked identical on GPU.
    int    metalType;
    // Subsurface scattering: the kernel uses a wrap-diffuse approximation
