...
private:
    ...
    // Viewport shadow-map pass. Dominates cost on dense geometry
    // (rebuilds a 1024-res shadow map every frame with immediate-mode
    // GL calls over all triangles). OFF by default; users can enable
    // for still-frame lighting review.
    bool   _vpEnableShadows = false;
    ...
    // Cached geometry VBO for 3D viewport draw_handle. Previously we
    // iterated all triangles in immediate mode on every tumble frame,
    // which dominated cost at 120k+ tris. The VBO uploads once per
    // scene change and every frame just issues glDrawArrays.
    // Layout: interleaved { float3 position, float3 normal, float2 uv }
    // stride = 32 bytes, 3 verts per triangle.
    GLuint _glGeoVBO = 0;
    size_t _glGeoVBOTriCount = 0;   // number of tris uploaded
    bool   _glGeoVBODirty = true;   // force rebuild on next draw_handle
    // Per-mesh offsets (in TRIANGLES, not verts) for material-aware draws.
    // Parallel to _scene->GetMeshes() iteration order. Rebuilt with VBO.
    struct GeoMeshRange { int firstTri; int triCount; int matId; };
    std::vector<GeoMeshRange> _glGeoMeshRanges;
    // Cached scene bounds, updated during VBO rebuild.
    float _glSceneMin[3] = { 0.f, 0.f, 0.f };
    float _glSceneMax[3] = { 0.f, 0.f, 0.f };
    bool  _glSceneBoundsValid = false;

    // Helpers. Defined in SpectralRenderIop.cpp.
    void _EnsureGeoVBO();
