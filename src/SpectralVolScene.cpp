// SpectralVolScene — merges multiple VDBRead + light inputs
// Created by Marten Blumen

#include "SpectralVolScene.h"
...
const char* const SpectralVolScene::CLASS = "SpectralVolScene";
static Op* buildVolScene(Node* node) { return new SpectralVolScene(node); }
const GeomOp::Description SpectralVolScene::description(CLASS, buildVolScene);

class SpectralVolScene::Engine : public SourceEngine
{
public:
    ...
    void createPrims(GeomSceneContext& context, const Path& path) override
    {
        ...
        prim.setCustomData("spectralIsVolScene", Value(true));
    }
};

SpectralVolScene::SpectralVolScene(Node* node)
    : SourceGeomOp(node, BuildEngine<Engine>()) {}

const char* SpectralVolScene::node_help() const
{
    return "SpectralVolScene -- Volume Scene Merge\n\n"
           ...
           "CONNECTION\n"
           "  VDBRead1 -> [GeoTransform] -> VolScene input 0\n"
           "  VDBRead2 -> [GeoTransform] -> VolScene input 1\n"
           "  EnvLight -> VolScene input 2\n"
           "  VolScene -> GeoScene -> SpectralRender\n\n"
           "Created by Marten Blumen\n"
           "github.com/bratgot/SpectralRenderer";
}

bool SpectralVolScene::test_input(int input, Op* op) const
{
    return dynamic_cast<GeomOp*>(op) != nullptr;
}

const char* SpectralVolScene::input_label(int input, char* buf) const
{
    sprintf(buf, "vol%d", input);
    return buf;
}

void SpectralVolScene::knobs(Knob_Callback f)
{
    ...
    Text_knob(f, "<font size='1' color='#555'>"
                 "SpectralVolScene - SpectralRenderer for Nuke 17"
                 "</font>");
}

std::vector<VolSceneEntry> SpectralVolScene::GetVolumes(int frame, int maxRes)
{
    std::vector<VolSceneEntry> result;
    _envLight = nullptr;
    ...
    for (int inp = 0; inp < inputs(); ++inp) {
        ...
        for (int depth = 0; depth < 4 && cur; ++depth) {
            ...
            fprintf(stderr, "VolScene: inp%d depth%d class=%s\n", inp, depth, cur->Class());
            ...
            if (strcmp(cur->Class(), "GeoTransform") == 0 ||
                ...
                foundXform = true;
                // Try standard Nuke knob names
                if (Knob* k = cur->knob("translate")) {
                    tx=k->get_value(0); ty=k->get_value(1); tz=k->get_value(2);
                } else if (Knob* k = cur->knob("xform_translate")) {
                    tx=k->get_value(0); ty=k->get_value(1); tz=k->get_value(2);
                }
                if (Knob* k = cur->knob("rotate")) {
                    rx=k->get_value(0); ry=k->get_value(1); rz=k->get_value(2);
                } else if (Knob* k = cur->knob("xform_rotate")) {
                    rx=k->get_value(0); ry=k->get_value(1); rz=k->get_value(2);
                }
                if (Knob* k = cur->knob("scaling")) {
                    scx=k->get_value(0); scy=k->get_value(1); scz=k->get_value(2);
                } else if (Knob* k = cur->knob("xform_scale")) {
                    scx=k->get_value(0); scy=k->get_value(1); scz=k->get_value(2);
                }
                fprintf(stderr, "VolScene: found %s T=(%.1f,%.1f,%.1f) R=(%.1f,%.1f,%.1f) S=(%.2f,%.2f,%.2f)\n",
                        cur->Class(), tx,ty,tz, rx,ry,rz, scx,scy,scz);
            }
        ...
        if (vdbRead && !vdbRead->node_disabled()) {
            ...
            if (vol && vol->IsValid()) {
                ...
                result.push_back(e);
                fprintf(stderr, "VolScene: inp%d vol %dx%dx%d bbox(%.1f,%.1f,%.1f)-(%.1f,%.1f,%.1f)%s\n",
